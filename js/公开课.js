window.onload = function() {
	...
	var current_index = 0;
	var timer = window.setInterval(autoChange, 1000000);
	for(var i = 0; i < head_divs.length; i++) {
		head_divs[i].onmouseover = function() {
			...
			var body_uls = document.getElementById("tab-body").getElementsByTagName("div");
		...
		head_divs[i].onmouseout = function() {
			timer = setInterval(autoChange, 1000000);
		}
		...
		function autoChange() {
			...
			for(var o = 0; i < head_divs.length; i++) {
				if(i == current_index) {
